...
class Order(Base):
    ...
    initiator_id = Column(Integer, ForeignKey(User.id, ondelete='SET NULL'), nullable=False,
                          doc="Идентификатор пользователя-инициатора заказа.")
    basket_history = Column(JSON, doc="История корзины заказа.")
    created_at = Column(TIMESTAMP, default=datetime.utcnow, doc="Метка времени создания заказа.")
