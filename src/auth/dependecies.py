...
def get_token(request: Request):
    ...
    if not token:
        ...
        if path.startswith("/pages"):
            ...
            raise HTTPException(
                status_code=307,
                detail=f"Redirect to {redirect_url}",
                headers={"Location": redirect_url},
            )
        else:
            raise HTTPException(
                status_code=401,
                detail="Access token is missing. You are not authenticated.",
            )
    return token
...
async def get_current_user(token: str = Depends(get_token)):
    try:
        ...
        raise HTTPException(
            status_code=401, detail="Token has expired. Please re-authenticate."
        )
    except jwt.DecodeError:
        raise HTTPException(
            status_code=401, detail="Token decoding error. Please re-authenticate."
        )
    ...
    if not user_id:
        raise HTTPException(
            status_code=401,
            detail="User identifier is missing in the token. Please re-authenticate.",
        )
    user = await UserDAO.find_by_id(int(user_id))
    if not user:
        raise HTTPException(
            status_code=401, detail="User with the specified identifier not found."
        )
    return user
